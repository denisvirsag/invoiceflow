...
export default async function DashboardPage() {
  ...
  for (const inv of invoices) {
    if (inv.status !== "template" && inv.status !== "cancelled" && inv.status !== "draft" && inv.issueDate.getMonth() === currentMonth && inv.issueDate.getFullYear() === currentYear) {
      revenue += inv.total;
    }
  ...
  // Prepare invoice data for chart (serializable)
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "var(--space-6)", marginTop: "var(--space-8)" }}>
        {/* Monthly trend chart */}
        <RevenueChart invoices={chartInvoices} />
